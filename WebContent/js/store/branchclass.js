...
function editClass(classid){
	  $.ajax({
	   	  type: "GET",
	   	  url: "store/branchclass/edit?classid="+classid,
	   	  success: function (data) {
		   	   	  $("#paper_type_dialog").html(data);
		   	      $("#paper_type_edit").modal("show");  
	   	  },
...
function submitClass(operation){
	...
	item.branchSidList = list;
	var url = "";
	if(operation=="insert"){
		url="store/branchclass/insert";
	}else{
		item.classid=$("#editclassid").val();
		url="store/branchclass/update";
	}
	  $.ajax({
	   	  type: "POST",
	   	  url: url,
	   	  ...
	   	  success: function (data) {
	   		  ...
	   		  if(jsonData.status==true){	   			   
		   	      ...
		   	      $("#operationMesModal").modal("show");
		   	      setTimeout("location.href='store/branchclass/list'",1500);
	   		  }else{
		   	   	  $("#modalMes").html(jsonData.message);
		   	      $("#operationMesModal").modal("show");
	   		  }
